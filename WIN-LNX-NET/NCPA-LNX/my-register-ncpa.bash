...
#-- Copy Nagios Plugins to remote
###/usr/bin/ssh -q -o BatchMode=yes  -o StrictHostKeyChecking=no nagios@${myipaddr} 'mkdir -p /home/nagios/libexec'
#scp $MYPWD/mynagplugins.tar.gz nagios@${myipaddr}:/home/nagios/
#/usr/bin/ssh -q -o BatchMode=yes  -o StrictHostKeyChecking=no nagios@${myipaddr} 'tar zxvfBp mynagplugins.tar.gz'
###scp -p $MYPWD/mynagplugins/* nagios@${myipaddr}:/home/nagios/libexec
###/usr/bin/ssh -q -o BatchMode=yes  -o StrictHostKeyChecking=no nagios@${myipaddr} 'chown -R apache:nagios /home/nagios/libexec/*'
###/usr/bin/ssh -q -o BatchMode=yes  -o StrictHostKeyChecking=no nagios@${myipaddr} 'chown root /home/nagios/libexec/check_dhcp'
###/usr/bin/ssh -q -o BatchMode=yes  -o StrictHostKeyChecking=no nagios@${myipaddr} 'chown root /home/nagios/libexec/check_icmp'
if [[ $MYOSGEN == "AIX" ]]; then
  scp -p $MYPWD/myaixlibexec/* nagios@${myipaddr}:/usr/local/ncpa/plugins/
fi
...
for j in $( grep -v '^#' ${myostypelist} ); do
  ...
  case $j in
    ...
    SOLARIS)
      ...
      MYOSGEN="$j"
      MYAGENT="NCPA"
      iconimage="solaris.png"
    ;;
    WINDOWS)
      MYHG="$j"
      MYOSGEN="$j"
      MYAGENT="NCPA"
      iconimage="win_server.png"
    ;;
    *)
      MYHG="$j"
      MYOSGEN="$j"
